...
const MainLayout = () => {
  ...
  return (
    <div className="max-w-screen flex min-h-screen flex-col bg-slate-50 overflow-x-hidden">
      <SidebarProvider>
        {!isMobile && <AppSidebar variant="inset" collapsible="icon" />}
        <div className="flex flex-grow flex-col">
          {isMobile && <AppSidebar variant="inset" collapsible="icon" />}
          <div
            className={`flex ${isMobile ? "max-w-[100vw]" : "max-w-[calc(100vw-var(--sidebar-width))]"}  grow items-start justify-start p-10 pt-14 overflow-hidden`}
          >
            <Outlet />
          </div>
